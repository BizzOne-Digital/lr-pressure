...
// Swaps a service's placeholder/stock photo for a real client photo — but
// only if the service's current image is still that exact stock photo (or
// missing entirely). If an admin has already uploaded a different image
// through the CMS, this leaves it alone rather than overwriting a real
// choice they made.
async function replaceServiceImageIfStock(
  slug: string,
  oldStockImageId: mongoose.Types.ObjectId,
  newImageId: mongoose.Types.ObjectId
) {
  const service = await Service.findOne({ slug });
  if (!service) {
    console.log(`Service "${slug}" not found, skipping real-photo update.`);
    return;
  }
  if (service.imageMediaId && service.imageMediaId.toString() !== oldStockImageId.toString()) {
    console.log(`Service "${slug}" already has a custom image, skipping real-photo update.`);
    return;
  }
  service.imageMediaId = newImageId;
  await service.save();
  console.log(`Replaced stock photo with a real client photo for service "${slug}".`);
}

async function addProjectIfMissing(project: {
  title: string;
  slug: string;
  summary: string;
  description: string;
  ...
  await Project.create({ ...project, active: true });
...
// Backfills the dedicated per-service "why choose us" checklist, process
// steps, and FAQs onto a service that doesn't have them yet — only when
// each field is still empty, so any admin edits made through the CMS are
// never overwritten. Safe to run repeatedly.
async function backfillServiceContentIfMissing(slug: string) {
  const entry = SERVICE_CONTENT[slug];
  if (!entry) {
    console.log(`No authored content found for service "${slug}", skipping.`);
    return;
  }
  const service = await Service.findOne({ slug });
  if (!service) {
    console.log(`Service "${slug}" not found, skipping content backfill.`);
    return;
  }
  let changed = false;
  if (!service.benefits?.length) {
    service.benefits = entry.benefits;
    changed = true;
  }
  if (!service.processSteps?.length) {
    service.processSteps = entry.processSteps;
    changed = true;
  }
  if (!service.faqs?.length) {
    service.faqs = entry.faqs;
    changed = true;
  }
  if (!changed) {
    console.log(`Service "${slug}" already has its own page content, skipping.`);
    return;
  }
  await service.save();
  console.log(`Backfilled page content (benefits/process/FAQs) for service "${slug}".`);
}
...
async function main() {
  ...
  // Backfill the dedicated per-service page content (checklist, process
  // steps, FAQs) that powers the Panther-matched individual service page
  // structure — one call per service, each guarded to skip a service that
  // already has its own content (whether from an earlier run of this
  // script or from an admin edit through the CMS).
  for (const slug of Object.keys(SERVICE_CONTENT)) {
    await backfillServiceContentIfMissing(slug);
  }

  console.log("");

  // ---------------------------------------------------------------------
  // Real client project photos (LR Pressure Washing's own before/after
  // work, provided directly by the client — not stock, so no CREDITS.md
  // attribution is needed). These replace the stock placeholder photo on
  // each matching service, and are also added as real Projects and Gallery
  // items so the actual work is showcased across the site.
  // ---------------------------------------------------------------------
  ...
  // Stock photos referenced here only so we can detect and replace them —
  // uploadLocalImage dedupes by filename, so these resolve to the same
  // Media documents already used as each service's current image.
  const stockConcreteAction = await uploadLocalImage("concrete-action.jpg", "High pressure concrete cleaning");
  const stockServiceConcrete = await uploadLocalImage("service-concrete.jpg", "Concrete surface cleaning");
  const stockServiceWindow = await uploadLocalImage("service-window.jpg", "Window cleaning close up");
  const stockHeroHouseWash = await uploadLocalImage("hero-house-wash.jpg", "Pressure washing a house exterior");

  await replaceServiceImageIfStock("interior-window-cleaning", stockServiceWindow, realInteriorWindow);
  await replaceServiceImageIfStock("patio-walkway-cleaning", stockConcreteAction, realPatioBeforeAfter);
  await replaceServiceImageIfStock("house-washing", stockHeroHouseWash, realSidingBeforeAfter);
  await replaceServiceImageIfStock("concrete-cleaning", stockServiceConcrete, realConcreteWalkwayBeforeAfter);
  await replaceServiceImageIfStock("exterior-surface-cleaning", stockConcreteAction, realStuccoChimneyBeforeAfter);

  console.log("");

  await addProjectIfMissing({
    title: "Interior Window Cleaning",
    slug: "interior-window-cleaning-project",
    summary: "A freshly cleaned interior window, letting in a clear, streak-free view.",
    description:
      "Interior window cleaning for a residential customer, leaving the glass streak-free and the view clear.",
  ...
  await addProjectIfMissing({
    title: "Paver Patio Cleaning",
    slug: "paver-patio-cleaning-project",
    summary: "A paver patio before and after a thorough pressure washing.",
    description:
      "This paver patio had built-up dirt and grime across the surface and joints. A full pressure wash restored the pavers to a clean, even appearance, ready for outdoor furniture and everyday use again.",
    imageMediaId: realPatioBeforeAfter,
    location: "",
    serviceSlug: "patio-walkway-cleaning",
    featured: true,
    order: 1,
  });

  await addProjectIfMissing({
    title: "Siding Soft-Wash",
    slug: "siding-soft-wash-project",
    summary: "Vinyl siding before and after a soft-wash cleaning removed algae and green staining.",
    description:
      "Green algae and mildew staining had built up on this home's vinyl siding over time. A soft-wash cleaning lifted the staining without damaging the siding, restoring an even, like-new appearance.",
  ...
  await addProjectIfMissing({
    ...
    slug: "concrete-walkway-cleaning-project",
    summary: "A concrete walkway before and after pressure washing removed algae and grime buildup.",
    description:
      "This concrete walkway had visible algae and grime buildup along its surface and edges. Pressure washing restored a clean, uniform look across the full length of the walkway.",
  ...
  await addProjectIfMissing({
    title: "Stucco Chimney Cleaning",
    slug: "stucco-chimney-cleaning-project",
    summary: "A stucco chimney before and after cleaning removed heavy dark staining.",
    description:
      "Heavy dark staining had built up on this stucco chimney over time. A thorough exterior cleaning removed the staining and restored the stucco's original, even color.",
    imageMediaId: realStuccoChimneyBeforeAfter,
    location: "",
    serviceSlug: "exterior-surface-cleaning",
    featured: false,
    order: 4,
  });
  ...
  const realPhotoGalleryCount = await GalleryItem.countDocuments();
  const realPhotoNextOrder = realPhotoGalleryCount;

  await addGalleryItemIfMissing({
    title: "Interior Window — Clear View",
    caption: "A freshly cleaned interior window with a clear, streak-free view",
    category: "Windows",
    imageMediaId: realInteriorWindow,
    order: realPhotoNextOrder,
    featured: false,
  });

  await addGalleryItemIfMissing({
    title: "Paver Patio — Before & After",
    caption: "Paver patio before and after pressure washing",
    category: "Before & After",
    imageMediaId: realPatioBeforeAfter,
    order: realPhotoNextOrder + 1,
    featured: true,
  });

  await addGalleryItemIfMissing({
    title: "Siding — Before & After",
    caption: "Vinyl siding before and after a soft-wash cleaning",
    category: "Before & After",
    imageMediaId: realSidingBeforeAfter,
    order: realPhotoNextOrder + 2,
    featured: true,
  });

  await addGalleryItemIfMissing({
    title: "Concrete Walkway — Before & After",
    caption: "Concrete walkway before and after pressure washing",
    category: "Before & After",
    imageMediaId: realConcreteWalkwayBeforeAfter,
    order: realPhotoNextOrder + 3,
    featured: false,
  });

  await addGalleryItemIfMissing({
    title: "Stucco Chimney — Before & After",
    caption: "Stucco chimney before and after cleaning",
    category: "Before & After",
    imageMediaId: realStuccoChimneyBeforeAfter,
    order: realPhotoNextOrder + 4,
    featured: false,
  });
