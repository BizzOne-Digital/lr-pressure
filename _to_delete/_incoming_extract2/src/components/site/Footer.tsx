      ...
      >
          ...
          <div className="font-heading text-xl font-bold text-white">
            {settings.businessName.split(" ").map((word, i) =>
              i === 0 ? (
                ...
                <span key={i} className="text-brand-red-light">
                  {word}{" "}
                </span>
              )
            )}
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5 text-sm">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link href={item.href} className="text-white/65 hover:text-brand-red-light">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5 text-sm">
            {serviceLinks.length > 0 ? (
              serviceLinks.map((s) => (
                <li key={s.slug}>
                  <Link href={`/services/${s.slug}`} className="text-white/65 hover:text-brand-red-light">
                    {s.name}
                  </Link>
                </li>
              ))
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm">
            <li>
              <a href={telHref} className="flex items-start gap-2.5 text-white/65 hover:text-brand-red-light">
            ...
            <li>
              <a
                ...
                className="flex items-start gap-2.5 text-white/65 hover:text-brand-red-light"
              >
            ...
            {settings.businessHours && (
              <li className="flex items-start gap-2.5 text-white/65">
                <Clock className="mt-0.5 h-4 w-4 shrink-0 text-brand-red-light" />
                {settings.businessHours}
              </li>
            )}
          </ul>
      ...
      <div className="border-t border-white/10">
        <div className="container-lux flex flex-col items-center justify-between gap-3 py-6 text-xs text-white/45 sm:flex-row">
          ...
          <div className="flex gap-5">
            <Link href="/privacy-policy" className="hover:text-brand-red-light">
              Privacy Policy
            </Link>
            <Link href="/terms" className="hover:text-brand-red-light">
              Terms
            </Link>
            <Link href="/sitemap" className="hover:text-brand-red-light">
              Sitemap
            </Link>
          </div>
        </div>
